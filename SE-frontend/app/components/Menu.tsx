...
const Menu = () => {
  return (
    <div className="mt-3 text-xl flex justify-center items-center">
      {menuItems.map((i) => (
        <div className="flex flex-col gap-5" key={i.title}>
          {i.items.map((item) => (
            <Link
              href={item.href}
              key={item.label}
              className="flex items-center justify-center lg:justify-start gap-4 text-black py-2 md:px-2 rounded-md hover:bg-gray-100"
            >
              {item.icon && item.icon}
